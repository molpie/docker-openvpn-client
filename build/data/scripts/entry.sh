...
# Create tun device node if missing
mkdir -p /dev/net
[ -c /dev/net/tun ] || mknod /dev/net/tun c 10 200

# Move the config files from the mounted volume to a writeable dir
cp -RL /vpn /data

# Capture the filename of the first .conf file to use as the OpenVPN config.
config_file_original=$(find /data/vpn -name "*.conf" -o -name "*.ovpn" 2> /dev/null | sort | head -1)
...
Proxy username secret: ${PROXY_USERNAME_SECRET:-none}
Proxy password secret: ${PROXY_PASSWORD_SECRET:-none}
Allowing subnets:      ${SUBNETS:-none}
Using config file:     $config_file_original
OpenVPN log level:     $vpn_log_level
"
...
if [ "$HTTP_PROXY" = "on" ]; then
    if [ "$PROXY_USERNAME" ]; then
        ...
        if [ -f "/run/secrets/$PROXY_PASSWORD_SECRET" ]; then
            echo "Configuring HTTP proxy authentication from secrets."
            printf "\nBasicAuth %s %s\n" \
                "$(cat /run/secrets/"$PROXY_USERNAME_SECRET")" \
                "$(cat /run/secrets/"$PROXY_PASSWORD_SECRET")" >> /data/tinyproxy.conf
        else
...
if [ "$SOCKS_PROXY" = "on" ]; then
    if [ "$PROXY_USERNAME" ]; then
        ...
        if [ -f "/run/secrets/$PROXY_PASSWORD_SECRET" ]; then
            echo "Configuring SOCKS proxy authentication from secrets."
            _user=$(cat /run/secrets/"$PROXY_USERNAME_SECRET")
            _pass=$(cat /run/secrets/"$PROXY_PASSWORD_SECRET")
            adduser -S -D -g "$_user" -H -h /dev/null "$_user"
            echo "$_user:$_pass" | chpasswd 2> /dev/null
            sed -i 's/socksmethod: none/socksmethod: username/' /data/sockd.conf
        else
...
if [ -n "${OPENVPN_AUTH_SECRET:-}" ]; then
    if [ -f "/run/secrets/$OPENVPN_AUTH_SECRET" ]; then
        ...
        echo "WARNING: OpenVPN credentials secret not found."
    fi
fi
